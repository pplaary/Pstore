...
### Commit 6: 清扫 P2 问题（一次性处理，避免后续审校反复列出）

**涉及文件**：多个

1. 删除死代码文件 `src/navigation/RootNavigator.tsx`（整文件未被引用）

2. App.tsx：删除重复的 RootStackParamList 定义，改为从 types.ts 导入
   - 删除 L11-16 的 type 定义
   - 顶部加 `import type { RootStackParamList } from './src/navigation/types';`

3. App.tsx L21：db 类型从 `ReturnType<typeof initDatabase> extends Promise<infer T> ? T : never` 改为 `SQLite.SQLiteDatabase`
   - 顶部加 `import * as SQLite from 'expo-sqlite';`
   - 状态声明改为 `useState<SQLite.SQLiteDatabase | null>(null)`
   - 删除 L68 的 `as any`

4. ProductEditScreen.tsx：删除未使用的 Field 组件（L246-264）

5. product.ts：所有 INSERT INTO product_fts 的 `last_insert_rowid()` 改为 `(SELECT rowid FROM product WHERE id = ?)` 子查询方式，与 fastRefresh 保持一致
   - 需改 addProduct 和 updateProduct 两处

6. init.ts：fastRefresh 的 SELECT 子查询方式已有则不动；确认 product.ts 改动后与 init.ts 的写法统一

提交信息：`fix(review): 清扫 P2 — 死代码、类型重复、last_insert_rowid 统一`

---

## 完成标准

1. `git log --oneline` 应显示 6 个新的 fix commit
2. 所有修改不破坏已有功能
